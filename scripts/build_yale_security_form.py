...
rows = [
    ...
    ("SEC 0.d", "Does your system/application support multi-factor authentication for administrative access to systems and its supporting infrastructure?", "Yes",
     "Yes. Admin (platform admin) accounts authenticate via Supabase Auth with mandatory TOTP MFA (authenticator app) plus 12+ character password. Same enforced MFA gate as all users."),
    ...
    ("SEC 2.g", "If assigned to individuals, what authentication mechanisms restrict access (AD login, MFA, etc.)?", "",
     "Individual Supabase Auth credentials: unique email + 12+ character password + mandatory TOTP MFA. Role enforced server-side via PostgreSQL Row-Level Security (RLS)."),
    ...
    ("SEC 3.h", "Does the system/application prohibit user accounts without passwords (no blank passwords)?", "Yes",
     "Yes. Minimum 12-character password enforced at four layers (Supabase Auth config, client, edge functions, legacy-user upgrade prompt). Blank/whitespace-only rejected."),
    ...
    ("SEC 3.j", "Does the system/application enforce password complexity? Specify criteria.", "Yes",
     "Yes. Minimum 12 characters (NIST 800-63B length-first guidance), with guidance/checks for uppercase, lowercase, number, and symbol; mostly-whitespace passwords rejected. Supabase enforces minimum length server-side. Note: compromised-password screening (HaveIBeenPwned) is available but not currently enabled (paid-tier feature); will enable on upgrade to Supabase Pro."),
    ...
    ("SEC 3.l", "Does the system/application enforce a password expiration interval? Specify.", "No",
     "No forced periodic expiration (aligned with NIST 800-63B, which discourages routine rotation). However, users whose passwords predate the current 12-char policy are force-prompted to upgrade at login."),
    ...
    ("SEC 11.a", "Are there additional security controls that you would like to highlight?", "Yes",
     "Highlights: (1) MANDATORY TOTP MFA for ALL users, enforced by a full-screen security gate so NO application content, navigation, or data renders until password policy, current Terms acceptance, and MFA are satisfied. (2) 12+ character password policy enforced at four layers. (3) PostgreSQL Row-Level Security for per-row, role-based data isolation. (4) Append-only audit_log and security_events (failed logins, MFA events, idle timeouts, password changes, sensitive-record changes) that cannot be modified/deleted by app roles. (5) 30-minute idle auto-logout for shared clinical workstations. (6) Strict CSP and security headers (X-Frame-Options: DENY, nosniff, HTTPS-only). (7) PHI prohibited by design (zero-tolerance Terms). (8) US-only hosting (AWS us-east-2). (9) Self-service password reset; refresh-token rotation; 1-hour token expiry."),
]
