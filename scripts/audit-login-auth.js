...
ok('12. resetPasswordForEmail uses Resend edge function (not Supabase SMTP)', authCtx.includes("functions.invoke('request-password-reset'") && !authCtx.includes('supabase.auth.resetPasswordForEmail'));
...
ok('19. Forgot-password calls resetPasswordForEmail (Resend path)', login.includes('resetPasswordForEmail'));
...
ok('24. Admin team reset emails use adminSendPasswordReset (Resend + copy link)', admin.includes('adminSendPasswordReset') && !admin.includes('supabase.auth.resetPasswordForEmail'));
...
ok('26. Runbook documents password reset + email branding', runbook.includes('Password reset') && (runbook.includes('Custom SMTP') || runbook.includes('Resend') || runbook.includes('no.reply@impactscollaborative.com')));
