...
async function addDefaultResources() {
  try {
    ...
    const defaultResources = [
      {
        resource_id: 'default-1',
        user_id: 'test@impacts.com',
    ...
    console.log('Adding default resources to BigQuery...');
    await table.insert(defaultResources);
    console.log('✅ Default resources added successfully!');
    
    // Verify the resources were added
    const query = `SELECT * FROM \`impacts-tracker.impacts_data.resources\` WHERE user_id = 'test@impacts.com'`;
    const [rows] = await bigquery.query(query);
    console.log(`\n📋 Found ${rows.length} resources for test@impacts.com:`);
