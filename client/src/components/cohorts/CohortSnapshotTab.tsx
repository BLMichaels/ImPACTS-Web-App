...
interface CohortSnapshotTabProps {
  cohortId: string;
  cohortName: string;
}

export const CohortSnapshotTab: React.FC<CohortSnapshotTabProps> = ({ cohortId, cohortName }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.error('Cohort snapshot error:', err);
  ...
  if (loading) {
    return (
      <Box sx={{ py: 4, textAlign: 'center' }}>
        ...
        <Typography color="text.secondary">Loading cohort snapshot...</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h6">Snapshot: {cohortName}</Typography>
        <Button size="small" startIcon={<RefreshIcon />} onClick={() => setRetryCount((c) => c + 1)}>
          Refresh
        </Button>
      </Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Metrics for mentors and PECCs in this cohort.
      </Typography>
      ...
      <Card>
        <CardContent>
          <Typography variant="subtitle1" fontWeight={600} gutterBottom>Mentors in cohort</Typography>
          {mentors.length === 0 ? (
            <Typography color="text.secondary" sx={{ py: 3 }}>No mentors in this cohort</Typography>
          ) : (
            <List>
...
export default CohortSnapshotTab;
